...
if st.sidebar.button("Submit"):
    ...
    st.json(decoded_predictions)

    # Visualization
    st.write("### Visualization of Predictions")
    plt.figure(figsize=(8, 6))
    plt.bar(decoded_predictions.keys(), [column_mappings[col][decoded_predictions[col]] for col in decoded_predictions], color='skyblue')
    plt.xlabel("Targets")
    plt.ylabel("Predicted Values")
    plt.title("Prediction Overview")
    st.pyplot(plt)
